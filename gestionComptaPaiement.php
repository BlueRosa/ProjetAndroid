...
<body>
<header>
    <a href="./seConnecter.php"><input type="button" value="se déconnecter"></a>
    <a href="./gestionSuperieur.php<?php if (isset($_GET['Sal_No'])){ echo "?Sal_No=".$_GET['Sal_No']; }?>"><input type="button" value="Validation des missions"></a>
    <input type="button" style="color: #555;" value="Paiement des frais"><a href="./gestionParametres.php<?php if (isset($_GET['Sal_No'])){ echo "?Sal_No=".$_GET['Sal_No']; }?>">
        <input type="button" value="paramètres"></a>
</header>
    ...
    if ($Sal_Personnel == 1) {
        ...
        foreach ($missions as $mission) {
            if ($mission['Mis_Remboursee']) {
                ...
                if ($cnt < count($distanceList)) {
                    if ($distanceList[$cnt]['Mis_No'] == $mission['Mis_No']){
                        echo '<a href="./gestionComptaPaiement.php?Mis_No='.$mission['Mis_No'].'&Sal_No='.$_GET['Sal_No'].'"><input type="button" value="Rembourser"></a> <br>';
                        $cnt++;
                    } else {
